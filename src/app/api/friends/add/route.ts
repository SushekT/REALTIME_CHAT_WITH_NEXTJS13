...
export async function POST(req: Request) {
  try {
    ...
    if (isAlreadyAdded){
        return new Response("Already Added. Please try with different user", {status: 400})
    }
    ...
    if (alreadyFriends){
        return new Response("You are Already Friend. Please try with different user", {status: 400})
    }
